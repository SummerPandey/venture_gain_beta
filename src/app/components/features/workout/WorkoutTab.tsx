...
import { Dumbbell, TrendingUp, Target, Award, Activity, ChevronUp, Zap, Plus, Minus, X, Camera, Mic, FolderOpen, Check, MessageCircle, Send } from 'lucide-react'
import { PixelBar } from '@/app/components/shared'
import { useWorkout } from '@/hooks'
import { groqText, groqVision, groqChat } from '@/lib/groq'
...
function maxWeightForExercise(logs: DayLog[], exercise: string): number {
  let max = 0
  for (const day of logs) {
    for (const w of day.workouts) {
      if (w.exercise === exercise && w.type === 'weights' && w.sets) {
        const dayMax = Math.max(...w.sets.map(s => s.weight))
        if (dayMax > max) max = dayMax
      }
      if (w.exercise === exercise && w.type === 'energy' && w.energyRating) {
        if (w.energyRating > max) max = w.energyRating
      }
    }
  }
  return max
...
export function WorkoutTab({ onEnterWorkoutMode }: { onEnterWorkoutMode?: () => void } = {}) {
  const {
    ...
    addSet, updateSet,
    logWorkout, logWorkoutDirect, adjustCardio,
  ...
  const { TODAY: _today } = (() => { const d = new Date(); return { TODAY: `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}` } })()
  const [scheduledTime, setScheduledTimeState] = useState<string | null>(() => localStorage.getItem(`workout_time_${_today}`))
  const [scheduledCategories, setScheduledCategoriesState] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem(`workout_cats_${_today}`) ?? '[]') } catch { return [] }
  })
  const setScheduledTime = (t: string | null) => {
    setScheduledTimeState(t)
    if (t) localStorage.setItem(`workout_time_${_today}`, t)
    else localStorage.removeItem(`workout_time_${_today}`)
  }
  const toggleScheduledCategory = (c: string) => {
    setScheduledCategoriesState(prev => {
      const next = prev.includes(c) ? prev.filter(x => x !== c) : [...prev, c]
      localStorage.setItem(`workout_cats_${_today}`, JSON.stringify(next))
      return next
    })
  }
...
- If nothing clear: [{"exercise":"Workout","type":"energy","sets":[],"energyRating":3,"description":"","unit":""}]`)
      const parsedRaw = JSON.parse(raw.replace(/```json|```/g, '').trim())
...
For all other messages just reply as plain text.`
      const raw = await groqChat(coachHistoryRef.current, systemPrompt)
  ...
  const exerciseProgressData = useMemo<ExerciseProgress[]>(() =>
    EXERCISE_GOALS.map(g => {
      const current = maxWeightForExercise(historicalLogs, g.exercise)
      return {
        ...g,
        current,
        progress: g.goal > 0 ? Math.min(100, Math.round((current / g.goal) * 100)) : 0,
      }
    }),
  [historicalLogs])
  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      ...
      <div className="monument-card p-4 mb-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            {editSavedFlash && (
              <span className="monument-text px-2 py-0.5" style={{ background: 'rgba(91,184,168,0.15)', border: '1px solid rgba(91,184,168,0.4)', borderRadius: '6px', color: '#369080', fontSize: '10px', fontWeight: '700' }}>✓ saved</span>
            )}
        ...
        {scheduledTime && scheduledCategories.length > 0 ? (
          ...
          <div className="mt-1">
              ...
              return (
                <div className="flex flex-col gap-1.5" style={{ maxHeight: 220, overflowY: 'auto' }}>
                  ...
                  {prev.map(exercise => {
                    ...
                    return (
                      <button key={exercise} onClick={() => { setSelectedExercise(exercise); setActiveLogCategory(cat?.name ?? scheduledCategories[0]) }}
                  ...
                  {rest.map(exercise => (
                    <button key={exercise} onClick={() => { setSelectedExercise(exercise); setActiveLogCategory(scheduledCategories[0]) }}
                      className="monument-button w-full flex items-center gap-2 px-3 py-2 flex-shrink-0"
            ...
            <button
              onClick={() => { setShowExerciseSelection(true); setSelectedWorkoutCategory(null); setCustomExerciseMode(false); setCustomExerciseName(''); setShowAllExercises(false); setExerciseSearch('') }}
      ...
      <div className="monument-card p-4 mb-4">
        ...
        <PixelBar label="WEEK" value={weeklyCardioMinutes} max={CARDIO_MAX_MINUTES} target={CARDIO_TARGET_MINUTES} color="#FFD4A8" unit="min" />
      </div>
      ...
      {selectedExercise && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(107, 68, 35, 0.7)', backdropFilter: 'blur(8px)' }} onClick={() => setSelectedExercise(null)}>
          <div className="monument-card p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => logWorkout(activeLogCategory)} className="monument-button w-full mb-4 py-3" style={{ background: 'linear-gradient(135deg, #FF9F66 0%, #FFB88A 100%)', borderRadius: '12px', border: '3px solid #8B5A3E', boxShadow: '0 4px 0 rgba(139, 90, 62, 0.25)', color: '#6B4423', fontSize: '11px', fontWeight: '700' }}>
              LOG WORKOUT
            </button>
      ...
      {/* Today's Workouts Detail Modal */}
      {viewingTodayWorkouts && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(107, 68, 35, 0.7)', backdropFilter: 'blur(8px)' }} onClick={() => setViewingTodayWorkouts(false)}>
          <div className="monument-card p-5 max-w-md w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <div className="monument-text" style={{ color: '#6B4423', fontSize: '13px', fontWeight: '700' }}>Today's Workouts</div>
              <button onClick={() => setViewingTodayWorkouts(false)} className="monument-button" style={{ ...BTN_BASE, color: '#6B4423', fontSize: '18px', fontWeight: '700', lineHeight: '1', width: '32px', height: '32px' }}>×</button>
            </div>
            {todayWorkouts.map((w, i) => (
              <div key={i} className="mb-3 px-3 py-2.5" style={{ background: 'rgba(255,252,248,0.6)', border: '1.5px solid rgba(139,90,62,0.18)', borderRadius: '12px' }}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="monument-text" style={{ color: '#6B4423', fontSize: '11px', fontWeight: '700' }}>{w.exercise}</span>
                  <div className="flex items-center gap-1.5">
                    <span className="monument-text" style={{ color: '#A0725A', fontSize: '9px', fontWeight: '700' }}>{w.time}</span>
                    <button
                      onClick={() => openWorkoutEdit(i)}
                      className="monument-button px-2 py-0.5"
                      style={{ background: 'rgba(255,252,248,0.95)', borderRadius: '6px', border: '1.5px solid #8B5A3E', color: '#8B5A3E', fontSize: '9px', fontWeight: '700' }}
                    >EDIT</button>
                    <button
                ...
                {w.type === 'energy' ? (
                  <div className="monument-text" style={{ color: '#8B5A3E', fontSize: '10px', fontWeight: '700' }}>Energy: {w.energyRating}/5</div>
                ) : (
                  <div className="flex flex-wrap gap-1.5">
                    {w.sets?.map((s, si) => {
                      const unit = exerciseProgressData.find(p => p.exercise === w.exercise)?.unit ?? 'kg'
                      return (
                        <span key={si} className="monument-text px-2 py-0.5" style={{ background: 'rgba(255,159,102,0.1)', border: '1px solid rgba(255,159,102,0.3)', borderRadius: '6px', color: '#8B5A3E', fontSize: '9px', fontWeight: '700' }}>
                          {si + 1}: {s.reps}r × {s.weight}{unit}
                        </span>
                      )
                    })}
                  </div>
                  ...
                  {isRestDay && (
                    <div className="flex flex-col items-center justify-center py-10 gap-3">
                      ...
                      <div className="monument-text text-center px-4" style={{ color: '#A0725A', fontSize: '10px', fontWeight: '700', lineHeight: 1.6 }}>
                        Rest days are when your body actually gets stronger. Mark today as a rest day to keep your streak going.
                      </div>
                      <button
                        onClick={() => { logWorkout('Rest Day'); setShowExerciseSelection(false); setSelectedWorkoutCategory(null) }}
                        className="monument-button px-6 py-3 mt-2"
